...
func DeployK8S(config *viper.Viper, tag string) {
	...
	masterinfo := temp.GetHostInfo(config, kubeMaster[0].(string))
	...
	// 设置admin用户集群参数
	cmd := fmt.Sprintf("/usr/local/bin/kubectl config set-cluster kubernetes --certificate-authority=/etc/kubernetes/kubernetesTLS/ca.pem --embed-certs=true --server=https://%s:6443", kubeMaster[0].(string))
	...
	cmd1 := fmt.Sprintf("/usr/local/bin/kubectl config set-cluster kubernetes --certificate-authority=/etc/kubernetes/kubernetesTLS/ca.pem --embed-certs=true --server=https://%s:6443 --kubeconfig=/etc/kubernetes/kube-proxy.kubeconfig", kubeMaster[0].(string))
	...
	cmd2 := fmt.Sprintf("/usr/local/bin/kubectl config set-cluster kubernetes --certificate-authority=/etc/kubernetes/kubernetesTLS/ca.pem --embed-certs=true --server=https://%s:6443 --kubeconfig=/etc/kubernetes/bootstrap.kubeconfig", kubeMaster[0].(string))
...
func deployNode(config *viper.Viper, node string, nodePkg, dockerPkg []interface{}, configConf string, wg *sync.WaitGroup) {
	...
	// 上传node二进制
	info := temp.GetHostInfo(config, node)
