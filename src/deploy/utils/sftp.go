...
func sftpConnect(host string, hostinfo temp.HostInfo) (*sftp.Client, error) {
...
func SFTPutFile(host string, hostinfo temp.HostInfo, src, dest string) {
...
func SFTPut(host string, hostinfo temp.HostInfo, src, dest string) {
	sftpClient, err := sftpConnect(host, hostinfo)
	if err != nil {
		log.Fatal(err)
	}
...
func SFTPFetchFile(host string, hostinfo temp.HostInfo, src, dest string) {
	sftpClient, err := sftpConnect(host, hostinfo)
	if err != nil {
		log.Fatal(err)
	}
